...
connect().then(database => {
    ...
    app.get('/environment/:dsluid/:module?', function (req, res, next) {
        let date = new Date(req.params.date);
        if (!req.params.module) {
            req.params.module = "MAP";
        }
        console.log('reached Environment dsluid ' + req.params.dsluid);
        res.send({EnvironmentUID: req.params.dsluid, DSL: []});
    });
